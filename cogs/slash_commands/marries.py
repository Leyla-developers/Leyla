...
class Marries(commands.Cog):
    ...
    async def marry_marries(self, inter):
        data = [f"`{self.bot.get_user(i['_id']).name}` + `{self.bot.get_user(i['mate']).name}` | <t:{i['time'].timestamp()}:D>" async for i in self.bot.config.DB.marries.find() if i['_id'] in [i.id for i in inter.guild.members] and i['mate'] in [i.id for i in inter.guild.members]]
