        ...
        if currency.upper() in data:
            ...
            await inter.send(
                embed=await self.bot.embeds.simple(
                    ...
                    description=f'{get_currency[upper_currency]} на данный момент стоит **{round(data[upper_currency]["Value"])}** рублей. (+{abs(round(data[upper_currency]["Value"] - data[upper_currency]["Previous"]))})',
                    fields=[
                        {
