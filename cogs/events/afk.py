...
class Afk(commands.Cog):
    ...
    async def on_message(self, message):
        ...
        if message.author.id in guild_afk_members:
            ...
            await message.channel.send(f"Смотрите, кто пришёл! {message.author.mention}, не устал(а) быть в AFK? | Вы пробыли в AFK <t:{round(data['time'].timestamp())}:R> | Причина: {data['reason']}")
        else:
            members = []
            ...
            for i in message.mentions:
                if i.id in guild_afk_members:
                    ...
                    return await message.channel.send(f'Ну, вообще-то, эт{"а" if len(members) == 1 else "и"} {"милашка" if len(members) == 1 else "милашки"} в AFK. Не думаю, что {"его(её)" if len(members) == 1 else "их"} стоит тревожить. ({"".join(members)})')
